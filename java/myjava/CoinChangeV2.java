...
public class CoinChangeV2 {
  public static int coinChange(int[] coins, int[] dp, int amount) {
    ...
    for (int coin : coins) {
      ...
      if (coinChange(coins, dp, amount - coin) == -1) continue;
  ...
  /*-
  if dp[i] == Integer.MAX_VALUE -> I don't know the answer yet
  if dp[i] == -1 -> it's impossible
  if dp[i] > 0 -> solution found for amount i
  */
