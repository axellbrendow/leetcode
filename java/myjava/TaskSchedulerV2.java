...
public class TaskSchedulerV2 {
  public static int leastInterval(char[] tasks, int n) {
    ...
    int cycle = 0;
    while (!frequencyHeap.isEmpty() || !cooldownQueue.isEmpty()) {
      cycle++;
      if (frequencyHeap.isEmpty()) cycle = cooldownMap.get(cooldownQueue.peek());
      while (!cooldownQueue.isEmpty() && cooldownMap.get(cooldownQueue.peek()) <= cycle) {
        final var cooldownTask = cooldownQueue.poll();
      ...
      if (frequencyMap.get(task) > 1) {
        frequencyMap.put(task, frequencyMap.get(task) - 1L);
        cooldownMap.put(task, cycle + n + 1);
        cooldownQueue.offer(task); // O(num of tasks * log(count of unique tasks))
      }
    }
    return cycle;
  }
